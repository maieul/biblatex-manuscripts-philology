...
\begin{itemize}
\item Provides new datatype: \bibtype{manuscripts} with adapted field
...
\cs{locationlibrarypunct} the punct between \bibfield{location} and \bibfield{library}.  By default \cs{addcolon}.

\cs{manuscriptdescriptionlabelpunct} the punct between label and text, for the special fields. By default \cs{addcolon}.

\cs{moreinterpunct} the punct between each special fields when printing in the same paragraph.

\cs{pagetotalpagespunct} the punct between \bibfield{pagetotal} and \bibfield{pages}. By default \cs{addcolon}.
\end{ltxsyntax}
...
\subsection{Macros and field format}

The \file{manuscripts.bbx} file defines bibmacro and field format (read the \biblatex handbook to know more about bibmacro and field format). We can't list all of them, but you can look on them to know how to customize more finely the manuscripts description.
\end{document}
