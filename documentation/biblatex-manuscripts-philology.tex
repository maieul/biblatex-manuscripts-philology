...
\csitem{datingpagespunct} the punct between \bibfield{dating} and \bibfield{pages}. By default \cs{addcomma}\cs{addspace}. The \cs{isdot} is automatically called when printing \bibfield{dating} field. Consequently, \cs{isdot} is deleted from \cs{datingpagespunct}.
...
\begin{release}{1.6.1}{2014-10-21}	
  	\item Add \cs{isdot} after each printing of the \bibfield{dating} field, to allow to use abbreviations with a dot (like \enquote{c.}) without adding a uppercase after.
\end{release}
